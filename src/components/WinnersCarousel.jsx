  ...
  return (
    <div className="glass p-4 rounded-2xl">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm small">This month</div>
      </div>
      ...
      <div ref={sc} className="mt-4 flex gap-3 overflow-x-auto no-scrollbar pb-2">
        {winners.length ? winners.map((w,i)=>(
          <div key={i} className="min-w-[180px] p-3 bg-white/3 rounded-lg">
            <div className="flex items-center gap-3">
              <img src={w.avatar||'/avatar-placeholder.png'} alt={w.name} className="w-12 h-12 rounded-full object-cover border border-white/10" />
              <div>
        ...
        )) : <div className="text-sm small">No winners yet</div>}
      </div>
    </div>
  );
}
